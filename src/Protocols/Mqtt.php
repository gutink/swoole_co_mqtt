 ...
 */
namespace GuMqtt\Protocols;

/**
 * Mqtt Protocol.
 * Fork：git@github.com:try-to/swoole_mqtt.git
 */
class Mqtt
{
	 ...
	 * @return array 拆分后的数据
	 */
	public static function splitBuffer( string $buffer ):array {
		$splitBufferArray = [];
		$bufferLength = strlen( $buffer );
		$startSite = 0;
		$surplusBuffer = $buffer;
		do{
			$bodyLength = static::getBodyLength( $surplusBuffer, $head_bytes );
			if( $bodyLength === 0 ){
				break;
			}
			//
			$splitBufferArray[] = substr( $surplusBuffer, 0, $head_bytes + $bodyLength );
			$surplusBuffer = substr( $surplusBuffer, $head_bytes + $bodyLength );
			//
		}while( $surplusBuffer !== '' );
		return $splitBufferArray;
	}
	...
	{
		$splitBuffer = static::splitBuffer($buffer);
		//
